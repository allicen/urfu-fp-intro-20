...
map f sample = foldr(\a b -> f a :. b) Nil sample
...
filter f list = foldr(\a b -> if f a == True then a :. b else b) Nil list
...
{-
 `reverse` разворачивает список:
  ...
  reverse f = foldl(\acc x -> x :. acc) Nil f
-}
reverse :: List a -> List a
reverse f = foldl (\a b -> \acc -> a (b:.acc)) id f Nil
...
intercalate :: [a] -> [[a]] -> [a]
intercalate sep [] = []
intercalate sep ls = P.foldr(\x acc -> x ++ sep ++ acc) [] (init ls) ++ last ls
